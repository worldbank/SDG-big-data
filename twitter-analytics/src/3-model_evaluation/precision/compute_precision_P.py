...
method='uncertainty_uncalibrated'
print(method)
method_dict={'our_method': 'convbert-', 'adaptive': 'adaptive', 'uncertainty': 'uncertainty', 'uncertainty_uncalibrated': 'uncalibrated'}
...
for folder_name in folder_list:
    ...
    for label in labels:
        ...
        if method=='our_method':
            rank = our_method_new_dict[iter_nb][label]['numerator']
        elif method=='adaptive':
            rank = adaptive_new_dict[iter_nb][label]['numerator']
        elif method=='uncertainty':
            rank = uncertainty_new_dict[iter_nb][label]['numerator']
        elif method=='uncertainty_uncalibrated':
            rank = uncertainty_uncalibrated_new_dict[iter_nb][label]['numerator']
        df = df.loc[df['rank'] < rank].reset_index(drop=True)
